...
{
	int ret = m_sock->send(buffer);//TODO:待优化，发送虽然成功但是不完整
...
{
	int ret = m_sock->sendto(buffer, addr);//TODO:待优化，发送虽然成功但是不完整
...
EServerParameter::EServerParameter(const std::string& ip, short port, ETYPE type,
	AcceptFunc acceptf, RecvFunc recvf, SendFunc sendf, RecvFromFunc recvfromf, SendToFunc sendtof)
{
	m_ip = ip;
	m_port = port;
	m_type = type;
