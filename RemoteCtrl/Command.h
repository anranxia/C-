...
protected:
	...
	unsigned threadid;
protected:
	static unsigned __stdcall threadLockDig(void* arg) {
		CCommand* thiz = (CCommand*)arg;
	...
	void threadLockDlgMain() {
		...
		while (GetMessage(&msg, NULL, 0, 0)) {
			...
			if (msg.message == WM_KEYDOWN) {
				...
				if (msg.wParam == 0x41) {//按下a键退出 ESC（0x1B）
					break;
				}
			}
		}
	...
	int SendScreen(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		...
		int nBitPerPixel = GetDeviceCaps(hScreen, BITSPIXEL);//24bit ARGB8888 32bit RGB888 24bit RGB565 RGB444
	...
	int LockMachine(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		if ((dig.m_hWnd == NULL) || (dig.m_hWnd == INVALID_HANDLE_VALUE)) {
			...
			_beginthreadex(NULL, 0, &CCommand::threadLockDig, this, 0, &threadid);
			TRACE("threadid=%d\r\n", threadid);
		}
	...
	int UnlockMachine(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		//dig.SendMessage(WM_KEYDOWN, 0x41, 0x01E0001);
		//::SendMessage(dig.m_hWnd, WM_KEYDOWN, 0x41, 0x01E0001);
	...
	int DeleteLocalFile(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		...
		//mbstowcs(sPath, strPath.c_str(), strPath.size());容易乱码
