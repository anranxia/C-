...
public:
	...
	int send(const EBuffer& buffer) { return ::send(m_socket, buffer, buffer.size(), 0); }
	int recv(EBuffer& buffer) { return ::recv(m_socket, buffer, buffer.size(), 0); }
	int sendto(const EBuffer& buffer, const ESockaddrIn& to) { return ::sendto(m_socket, buffer, buffer.size(), 0, to, to.size()); }
